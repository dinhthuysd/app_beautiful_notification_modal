...
class TelegramService:
    def __init__(self, bot_token: Optional[str] = None, admin_chat_id: Optional[str] = None):
        ...
        self.admin_chat_id = admin_chat_id or os.environ.get('TELEGRAM_ADMIN_CHAT_ID')
        self.bot = None
    ...
    def update_credentials(self, bot_token: Optional[str] = None, admin_chat_id: Optional[str] = None):
        """Update Telegram bot credentials dynamically"""
        if bot_token:
            self.bot_token = bot_token
            try:
                self.bot = Bot(token=self.bot_token)
                logger.info("✅ Telegram bot credentials updated")
            except Exception as e:
                logger.error(f"❌ Failed to update Telegram bot: {e}")
                self.bot = None
